...
Base.prototype.findBy = function(attrName, attrValue) {
  ...
  return Q.Promise(function(resolve, reject) {
    ...
    ref.once('value', function(snap) {
      if (!snap.exists()) {
        // TODO: Throw appropriate error object.
        reject(null);
      } else {
        var instanceSnapKey = _.keys(snap.val())[0];
        var instanceSnapVal = _.values(snap.val())[0];
        var instance = toBaseInstance(self, instanceSnapVal, instanceSnapKey);
        instance._internals.self = snap.child(instanceSnapKey).ref();
        resolve(instance);
      }
    }, function(err) {
      reject(err);
